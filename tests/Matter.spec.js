...
test("TC-78. Create Legal Matter and Additional Details 1st SFL", async () => {
  ...
  await matterPage.clickSaveForLater();
  await expect(matterPage.matterRowByTitle(data.caseTitle)).toBeVisible();
  await expect(matterPage.matterRowByTitle(data.caseTitle)).toContainText(
    data.caseTitle,
  );
});
...
test("TC-94. Add an existing client while creating a matter", async () => {
  const data = matterSearch.TC_CLIENT_001;
  await navigateToClientStep();
  await matterPage.enterClientName(data.existingClient);
  await matterPage.selectClient(data.existingClient);
  await matterPage.verifySelectedClient(data.existingClient);
});

test("TC-95. Search for a client that does not exist", async () => {
  const data = matterSearch.TC_CLIENT_002;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.verifyClientNotFound(data.searchNotFound);
});

test("TC-96. Verify new client form is displayed when client is not found", async () => {
  const data = matterSearch.TC_CLIENT_003;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.verifyNewClientFormDisplayed(data.searchNotFound);
});

test("TC-97. Create a new Individual client with valid details", async () => {
  const data = matterSearch.TC_CLIENT_004;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
    data.lastName,
    data.email,
    data.confirmEmail,
  ...
  await matterPage.clickAddAsClient();
  await matterPage.verifyClientAddedToSelected(data.firstName);
});

test("TC-98. Create a new Entity client with valid details", async () => {
  const data = matterSearch.TC_CLIENT_005;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillEntityClient(
    data.firmName,
    data.contactPerson,
    data.email,
    data.confirmEmail,
  ...
  await matterPage.clickAddAsClient();
  await matterPage.verifyClientAddedToSelected(data.firmName);
});

test("TC-99. Verify First Name is mandatory", async () => {
  const data = matterSearch.TC_CLIENT_006;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    "",
  ...
  await matterPage.clickAddAsClient();
  await matterPage.verifyValidationMessagePresent("First Name");
});

test("TC-100. Verify Last Name is mandatory", async () => {
  const data = matterSearch.TC_CLIENT_007;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
  ...
  await matterPage.clickAddAsClient();
  await matterPage.verifyValidationMessagePresent("Last Name");
});

test("TC-101. Verify Email Address is mandatory", async () => {
  const data = matterSearch.TC_CLIENT_008;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
  ...
  await matterPage.clickAddAsClient();
  await matterPage.verifyValidationMessagePresent("Email");
});

test("TC-102. Verify Confirm Email is mandatory", async () => {
  const data = matterSearch.TC_CLIENT_009;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
  ...
  await matterPage.clickAddAsClient();
  await matterPage.verifyValidationMessagePresent("Confirm Email");
});

test("TC-103. Verify Country is mandatory", async () => {
  const data = matterSearch.TC_CLIENT_010;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.selectIndividualClientType();
  await matterPage.individualFirstNameInput.fill(data.firstName);
  await matterPage.individualLastNameInput.fill(data.lastName);
  await matterPage.clientEmailInput.fill(data.email);
  await matterPage.clientConfirmEmailInput.fill(data.confirmEmail);
  await matterPage.clientPhoneNumberInput.fill(data.phone);
  await matterPage.clickAddAsClient();
  await matterPage.verifyValidationMessagePresent("Country");
});

test("TC-104. Verify email format validation", async () => {
  const data = matterSearch.TC_CLIENT_011;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
...
test("TC-105. Verify email confirmation mismatch", async () => {
  const data = matterSearch.TC_CLIENT_012;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
...
test("TC-106. Verify valid matching email addresses", async () => {
  const data = matterSearch.TC_CLIENT_013;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
...
test("TC-107. Verify Phone Number is optional", async () => {
  const data = matterSearch.TC_CLIENT_014;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
...
test("TC-108. Verify invalid phone number", async () => {
  const data = matterSearch.TC_CLIENT_015;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
  ...
  await matterPage.clickAddAsClient();
  await matterPage.verifyValidationMessagePresent("Phone");
});

test("TC-109. Verify special characters in First Name", async () => {
  const data = matterSearch.TC_CLIENT_016;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
  ...
  await matterPage.clickAddAsClient();
  const hasError = await matterPage.page
    .locator(".error:visible, .validation-error:visible")
    .count();
  expect(hasError).toBe(0);
});

test("TC-110. Verify spaces in client name", async () => {
  const data = matterSearch.TC_CLIENT_017;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
  ...
  await matterPage.clickAddAsClient();
  const firstNameValue = await matterPage.individualFirstNameInput.inputValue();
  expect(firstNameValue).toBe(firstNameValue.trim());
});

test("TC-111. Verify duplicate client creation", async () => {
  const data = matterSearch.TC_CLIENT_018;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
...
test("TC-112. Cancel new client creation", async () => {
  const data = matterSearch.TC_CLIENT_019;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
  ...
  await matterPage.clickCancelNewClient();
  await matterPage.verifyClientNotAdded(data.firstName);
});

test("TC-113. Cancel without entering any details", async () => {
  const data = matterSearch.TC_CLIENT_020;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.clickCancelNewClient();
});

test("TC-114. Switch from Individual to Entity", async () => {
  const data = matterSearch.TC_CLIENT_021;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.selectIndividualClientType();
  await matterPage.individualFirstNameInput.fill(data.firstName);
  await matterPage.selectEntityClientType();
  await matterPage.confirmClientTypeSwitch();
  await matterPage.verifyEntityFormVisible();
});

test("TC-115. Switch from Entity to Individual", async () => {
  const data = matterSearch.TC_CLIENT_022;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.selectEntityClientType();
  await matterPage.entityFirmNameInput.fill(data.firmName);
  await matterPage.selectIndividualClientType();
  await matterPage.confirmClientTypeSwitch();
  await matterPage.verifyIndividualFormVisible();
});

test("TC-116. Verify selected client appears after creation", async () => {
  const data = matterSearch.TC_CLIENT_023;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
...
test("TC-117. Verify multiple clients can be selected", async () => {
  const data = matterSearch.TC_CLIENT_024;
  await navigateToClientStep();
  await matterPage.enterClientName(data.existingClient);
  await matterPage.selectClient(data.existingClient);
  await matterPage.verifySelectedClient(data.existingClient);
...
test("TC-118. Remove a selected client", async () => {
  const data = matterSearch.TC_CLIENT_025;
  await navigateToClientStep();
  await matterPage.enterClientName(data.existingClient);
  await matterPage.selectClient(data.existingClient);
  await matterPage.verifySelectedClient(data.existingClient);
  await matterPage.removeSelectedClient(data.existingClient);
  await matterPage.verifyClientNotAdded(data.existingClient);
});

test("TC-119. Remove one client when multiple clients are selected", async () => {
  const data = matterSearch.TC_CLIENT_026;
  await navigateToClientStep();
  await matterPage.enterClientName(data.existingClient);
  await matterPage.selectClient(data.existingClient);
  await matterPage.verifySelectedClient(data.existingClient);
...
test("TC-120. Search using partial client name", async () => {
  const data = matterSearch.TC_CLIENT_027;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchKeyword);
  const results = matterPage.page
    .getByText(data.searchKeyword, { exact: false })
    .first();
  await expect(results).toBeVisible({ timeout: 10000 });
});

test("TC-121. Search with empty client field", async () => {
  await navigateToClientStep();
  const input = matterPage.clientSearchInput;
  await expect(input).toBeVisible({ timeout: 15000 });
  await input.fill("");
  await matterPage.clickClientSearch();
});

test("TC-122. Search with special characters", async () => {
  const data = matterSearch.TC_CLIENT_029;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchKeyword);
  await matterPage.verifyClientNotFound(data.searchKeyword);
});

test("TC-123. Verify newly created client can be used in matter creation", async () => {
  const data = matterSearch.TC_CLIENT_030;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
...
test("TC-124. Verify client data persistence", async () => {
  const data = matterSearch.TC_CLIENT_031;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.email);
  await matterPage.verifyNewClientFormVisible();
});

test("TC-125. Verify Add as Client cannot submit incomplete form", async () => {
  const data = matterSearch.TC_CLIENT_032;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.selectIndividualClientType();
  await matterPage.clickAddAsClient();
...
test("TC-126. Verify form data is retained after validation failure", async () => {
  const data = matterSearch.TC_CLIENT_033;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
...
test("TC-127. Verify client search after creating new client", async () => {
  const data = matterSearch.TC_CLIENT_034;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchKeyword);
  const result = matterPage.page
    .getByText(data.searchKeyword, { exact: true })
    .first();
...
test("TC-128. Verify error handling when client creation fails", async () => {
  const data = matterSearch.TC_CLIENT_035;
  await navigateToClientStep();
  await matterPage.searchClientByName(data.searchNotFound);
  await matterPage.fillIndividualClient(
    data.firstName,
...
test("TC-129. Create Legal Matter with Not-Found Client and Upload Documents - Solo Lawyer", async () => {
  const data =
    matterData1.TC131_CreateMatterWithNotFoundClient_NoAdditionalDetails_SoloLawyer;
  const uploadFiles = data.document.multiUploadFiles || [
    data.document.filePath,
  ];
  ...
  await matterPage.searchClientByName(data.newClient.searchNotFound);
  await matterPage.verifyClientNotFound(data.newClient.searchNotFound);
  await matterPage.verifyNewClientFormDisplayed(data.newClient.searchNotFound);
  await matterPage.fillIndividualClient(
    data.newClient.firstName,
    data.newClient.lastName,
    data.newClient.email,
    data.newClient.confirmEmail,
    data.newClient.country,
    data.newClient.phone,
  );
  await matterPage.clickAddAsClient();
  const fullName = `${data.newClient.firstName} ${data.newClient.lastName}`;
  await expect(
    matterPage.page.getByText(fullName, { exact: true }).first(),
  ...
  await matterPage.verifyClientPresentInListing(fullName, data.caseTitle);
  if (matterNumber) {
    await expect(
      matterPage.page.getByText(matterNumber, {
        exact: true,
...
test("TC-130. Create Legal Matter with Not-Found Client and Upload Documents - Entity", async () => {
  const data =
    matterData1.TC132_CreateMatterWithNotFoundClient_NoAdditionalDetails_Entity;
  const uploadFiles = data.document.multiUploadFiles || [
    data.document.filePath,
  ];
...
test("TC-31. Create Legal Matter with Additional Details, Not-Found Client and Upload Documents - Solo Lawyer", async () => {
  const data = matterData1.TC129_CreateMatterWithNotFoundClient_SoloLawyer;
  const uploadFiles = data.document.multiUploadFiles || [
    data.document.filePath,
  ];
...
test("TC-132. Create Legal Matter with Additional Details, Not-Found Client and Upload Documents - Entity", async () => {
  const data = matterData1.TC130_CreateMatterWithNotFoundClient_Entity;
  const uploadFiles = data.document.multiUploadFiles || [
    data.document.filePath,
  ];
...
test("TC-133. Create Legal Matter with Existing Client and Add New Individual Temporary Client", async () => {
  const data = matterData1.TC133_ExistingClientAndNewIndividualTemporaryClient;
  const uploadFiles = data.document.multiUploadFiles || [
    data.document.filePath,
  ];
  ...
  await matterPage.clickAddAsClient();
  const newClientFullName = `${data.newClient.firstName} ${data.newClient.lastName}`;
  await expect(
    matterPage.page.getByText(newClientFullName, { exact: true }).first(),
  ).toBeVisible({ timeout: 20000 });
...
test("TC-134. Create Legal Matter with Existing Client and Add New Entity Temporary Client", async () => {
  const data = matterData1.TC134_ExistingClientAndNewEntityTemporaryClient;
  const uploadFiles = data.document.multiUploadFiles || [
    data.document.filePath,
  ];
  ...
  await matterPage.verifyNewClientFormDisplayed(data.newClient.searchNotFound);
  await matterPage.fillEntityClient(
    data.newClient.firmName,
    data.newClient.contactPerson,
    data.newClient.email,
    data.newClient.confirmEmail,
    data.newClient.country,
    data.newClient.phone,
  ...
  await matterPage.clickAddAsClient();
  await expect(
    matterPage.page.getByText(data.newClient.firmName, { exact: true }).first(),
  ).toBeVisible({ timeout: 20000 });
...
test("TC-135. Create Legal Matter with Not-Found Individual Client and Save for Later", async () => {
  const data = matterData1.TC135_NotFoundIndividualClient_SaveForLater;
...
test("TC-136. Create Legal Matter with Not-Found Entity Client and Save for Later", async () => {
  const data = matterData1.TC136_NotFoundEntityClient_SaveForLater;
