...
class ClientBookingPage extends BasePage {
  constructor(page) {
    super(page);
  ...
  async bookLawyer(lawyerName) {
    ...
    for (let i = 0; i < maxPages; i++) {
      const card = this.lawyerCards.filter({
        has: this.page.locator(".lawyer-name", { hasText: nameRegex }),
      });

      if ((await card.count()) > 0) {
        const bookBtn = card.first().locator(".book-btn");
  ...
  async verifyAppointmentListed(text) {
    const row = this.appointmentRowByExactName(text).first();
    await expect(row, `Appointment with "${text}" not listed`).toBeVisible({
      timeout: 30000,
    });
  }
  ...
  appointmentRowByExactName(name) {
    const escaped = String(name).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    return this.page
      .locator(
        'tr, [role="row"], .card-item, .appointment-item, [class*="appointment"], li',
      )
      .filter({
        has: this.page.getByText(new RegExp(`^\\s*${escaped}\\s*$`, "i")),
      });
  }
  ...
  async verifyAppointmentOnLawyerSide(clientName, slotTime) {
    const row = this.appointmentRowByExactName(clientName).first();

    // Auto-retries to absorb sync delay between booking and dashboard
    await expect(
      row,
...
module.exports = ClientBookingPage;
